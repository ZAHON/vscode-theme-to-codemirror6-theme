...
export const Color = () => {
  ...
  const color = useSelector((state: RootState) => state.highlightStyle.tags.parenTag.color);
  const defaultColor = useSelector(
    (state: RootState) => state.highlightStyle.defaultTags.parenTag.color
  );

  const onChange = (value: string) => {
    dispatch(setHighlightStyleTag({ key: 'parenTag', property: 'color', value: value }));
  };
