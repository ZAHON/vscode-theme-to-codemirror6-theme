...
export const Color = () => {
  ...
  const color = useSelector((state: RootState) => state.highlightStyle.tags.operatorTag.color);
  const defaultColor = useSelector(
    (state: RootState) => state.highlightStyle.defaultTags.operatorTag.color
  );

  const onChange = (value: string) => {
    dispatch(setHighlightStyleTag({ key: 'operatorTag', property: 'color', value: value }));
  };
