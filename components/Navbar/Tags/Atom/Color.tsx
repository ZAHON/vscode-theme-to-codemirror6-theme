...
export const Color = () => {
  ...
  const color = useSelector((state: RootState) => state.highlightStyle.tags.atomTag.color);
  const defaultColor = useSelector(
    (state: RootState) => state.highlightStyle.defaultTags.atomTag.color
  );

  const onChange = (value: string) => {
    dispatch(setHighlightStyleTag({ key: 'atomTag', property: 'color', value: value }));
  };
