...
export const Color = () => {
  ...
  const color = useSelector((state: RootState) => state.highlightStyle.tags.stringTag.color);
  const defaultColor = useSelector(
    (state: RootState) => state.highlightStyle.defaultTags.stringTag.color
  );

  const onChange = (value: string) => {
    dispatch(setHighlightStyleTag({ key: 'stringTag', property: 'color', value: value }));
  };
