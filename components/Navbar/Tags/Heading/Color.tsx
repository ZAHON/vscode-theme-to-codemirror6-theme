...
export const Color = () => {
  ...
  const color = useSelector((state: RootState) => state.highlightStyle.tags.headingTag.color);
  const defaultColor = useSelector(
    (state: RootState) => state.highlightStyle.defaultTags.headingTag.color
  );

  const onChange = (value: string) => {
    dispatch(setHighlightStyleTag({ key: 'headingTag', property: 'color', value: value }));
  };
