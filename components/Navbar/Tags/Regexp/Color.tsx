...
export const Color = () => {
  ...
  const color = useSelector((state: RootState) => state.highlightStyle.tags.regexpTag.color);
  const defaultColor = useSelector(
    (state: RootState) => state.highlightStyle.defaultTags.regexpTag.color
  );

  const onChange = (value: string) => {
    dispatch(setHighlightStyleTag({ key: 'regexpTag', property: 'color', value: value }));
  };
