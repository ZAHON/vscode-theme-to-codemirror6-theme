...
export const Color = () => {
  ...
  const color = useSelector((state: RootState) => state.highlightStyle.tags.keywordTag.color);
  const defaultColor = useSelector(
    (state: RootState) => state.highlightStyle.defaultTags.keywordTag.color
  );

  const onChange = (value: string) => {
    dispatch(setHighlightStyleTag({ key: 'keywordTag', property: 'color', value: value }));
  };
